...
\emph{ESP8266} je levný WiFi mikročip, který s přidáním několika komponentů je schopen plnohodnotně plnit úlohu mikrokontroléru. Tento mikročip je velice oblíbený v oblasti chytrých zařízení díky své stabilititě, výkonosti, velikosti, ceně a mnoho dalšího.
Na trhu existuje spousty modulů, které obsahují právě zmíněný mikročip a plní tak úlohu mikrokontroléru. Mezi nejoblíbenější moduly, které obsahují tento mikročip jsou \emph{ESP-01}, který obsahyje pouze 2 GPIO piny, poté větší \emph{Wemos D1 mini} a \emph{NodeMCU}.
Tento projekt chytré domácnost je sestaven hlavně z těchto modulů díky svým přednostem a využití. Pro maximálně 2 jednotlivé vstupní/výstupní periferie připojené k modulu je využit modul \emph{ESP-01}. Pro komplexní využítí a připojení více periferií je použit modul \emph{Wemos D1 mini}.


\begin{figure}[hbt]
  \centering

  \begin{subfigure}[hbt]{.5\linewidth}
    \centering
    \includegraphics[width=.5 \linewidth]{obrazky-figures/esp-01.png}
    \caption{ESP-01 modul}
    \label{figure:esp01}
  \end{subfigure}
  ...
  \begin{subfigure}[hbt]{.5\linewidth}
    \centering
    \includegraphics[width=.5 \linewidth]{obrazky-figures/wemos.png}
    \caption{Wemos D1 mini modul}
    \label{figure:wemos}
  \end{subfigure}
