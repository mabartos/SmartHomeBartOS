...
O přijímání požadavku se stará implementace rozhraní \emph{JAX-RS}\footnote{\text{JAX-RS} - Java API for RESTful Web Services(rozhraní pro \emph{RESTful} služby)} a to přesněji \emph{RESTeasy}.
...
\begin{lstlisting}[caption={Ukázka přesměrování požadavku}]
...
\begin{lstlisting}[caption={Ukázka definování entity}]
  $$@Entity
  %%@Table%%(name = "Homes")
...
\begin{lstlisting}[caption={Ukázka asociace 1:N (domácnost:místnosti)}]
  // Entita domacnost
